...
def line_plot(df):
    """
    Renders a line plot interface in Streamlit with conditional UI elements.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(num_cat) > 2:
        ...
        with col1:
            selected_x = st.selectbox("Select X coordinate", num_cat, key='line_x_reg')
        with col2:
            selected_y = st.selectbox("Select Y coordinate", num_cat, key='line_y_reg')

        huee, colorss = None, None
        box = False

        if len(alp_cat) > 1:
            box = st.checkbox("Tick to activate hue", key='line_hue_check_reg')
            if box:
                ...
                with col1_hue:
                    ...
                    huee = st.selectbox("Select One", options=alp_cat, key='line_hue_select_reg')
                with col2_hue:
                    st.write("❄️ Change color theme of chart")
                    colorss = st.selectbox("Select Pallete", options=palette_list, key='line_palette_select_reg')
        else:
            st.info("ℹ️ Not enough categorical features available to use the 'hue' option.")

        if st.button("Generate Line Plot", key='line_submit_reg'):
...
def scatter_plot(df):
    """
    Renders a scatter plot interface in Streamlit with conditional UI elements.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(num_cat) > 2:
        col1, col2 = st.columns(2)
        with col1:
            selected_x = st.selectbox("Select X coordinate", num_cat, key='scatter_x_reg')
        with col2:
            selected_y = st.selectbox("Select Y coordinate", num_cat, key='scatter_y_reg')

        huee, colorss = None, None
        box = False

        if len(alp_cat) > 1:
            box = st.checkbox("Tick to activate hue", key='scatter_hue_check_reg')
            if box:
                palette_list = ['tab10', 'Set1', 'Set2', 'Set3', 'Paired', 'Pastel1', 'Dark2', 'colorblind', 'viridis',
                                'plasma', 'inferno', 'magma', 'cividis', 'Blues', 'Greens', 'Reds', 'YlOrRd', 'rocket', 'mako',
                                'coolwarm', 'bwr', 'seismic', 'RdBu', 'Spectral', 'vlag']
                ...
                with col1_hue:
                    st.write("🏹 See chart multiple categories in one click")
                    huee = st.selectbox("Select One", options=alp_cat, key='scatter_hue_select_reg')
                with col2_hue:
                    st.write("❄️ Change color theme of chart")
                    colorss = st.selectbox("Select Pallete", options=palette_list, key='scatter_palette_select_reg')
        else:
            st.info("ℹ️ Not enough categorical features available to use the 'hue' option.")

        if st.button("Generate Scatter Plot", key='scatter_submit_reg'):
            if selected_x != "Choices" and selected_y != "Choices":
                fig, ax = plt.subplots()
                if box and huee is not None and huee != "Choices":
...
def histogram_plot(df):
    """
    Renders a histogram plot interface in Streamlit with conditional UI elements.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(num_cat) > 1:
        selected = st.selectbox("Select a feature", num_cat, key='hist_x_reg')

        box = False
        huee, colorss = None, None

        if len(alp_cat) > 1:
            box = st.checkbox("Tick to activate hue", key='hist_hue_check_reg')
            if box:
                palette_list = ['tab10', 'Set1', 'Set2', 'Set3', 'Paired', 'Pastel1', 'Dark2', 'colorblind', 'viridis',
                                'plasma', 'inferno', 'magma', 'cividis', 'Blues', 'Greens', 'Reds', 'YlOrRd', 'rocket', 'mako',
                                'coolwarm', 'bwr', 'seismic', 'RdBu', 'Spectral', 'vlag']
                ...
                with col1:
                    ...
                    huee = st.selectbox("Select Hue", options=alp_cat, key='hist_hue_select_reg')
                with col2:
                    st.write("❄️ Change the color theme.")
                    colorss = st.selectbox("Select Palette", options=palette_list, key='hist_palette_select_reg')
        else:
            st.info("ℹ️ Not enough categorical features available to use the 'hue' option.")

        if st.button("Generate Histogram", key='hist_submit_reg'):
            if selected != "Choices":
                fig, ax = plt.subplots()
                if box and huee is not None and huee != "Choices":
...
def barchart_plot(df):
    """
    Renders a bar chart interface in Streamlit with conditional UI elements.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(num_cols) > 1 and len(cat_cols) > 1:
        col1, col2 = st.columns(2)
        with col1:
            selected_x = st.selectbox("Select X coordinate (Categorical)", cat_cols, key='bar_x_reg')
        with col2:
            selected_y = st.selectbox("Select Y coordinate (Numerical)", num_cols, key='bar_y_reg')

        box = False
        huee, colorss = None, None

        if len(cat_cols) > 2:
            box = st.checkbox("Tick to activate hue", key='bar_hue_check_reg')
            if box:
                palette_list = ['tab10', 'Set1', 'Set2', 'Set3', 'Paired', 'Pastel1', 'Dark2', 'colorblind', 'viridis',
                                'plasma', 'inferno', 'magma', 'cividis', 'Blues', 'Greens', 'Reds', 'YlOrRd', 'rocket', 'mako',
                                'coolwarm', 'bwr', 'seismic', 'RdBu', 'Spectral', 'vlag']
                ...
                with c1:
                    ...
                    huee = st.selectbox("Select Hue", options=hue_options, key='bar_hue_select_reg')
                with c2:
                    st.write("❄️ Change the color theme.")
                    colorss = st.selectbox("Select Palette", options=palette_list, key='bar_palette_select_reg')
        else:
            st.info("ℹ️ You need at least two categorical features to use the 'hue' option.")

        if st.button("Generate Bar Plot", key='bar_submit_reg'):
            if selected_x != "Choices" and selected_y != "Choices":
                fig, ax = plt.subplots()
                if box and huee is not None and huee != "Choices":
...
def piechart_plot(df):
    """
    Renders a pie chart interface in Streamlit with a conditional UI element.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(cat_cols) > 1:
        selected_col = st.selectbox("Select a feature", cat_cols, key='pie_select_reg')

        palette_list = ['tab10', 'Set1', 'Set2', 'Set3', 'Paired', 'Pastel1', 'Dark2', 'colorblind', 'viridis',
                        'plasma', 'inferno', 'magma', 'cividis', 'Blues', 'Greens', 'Reds', 'YlOrRd', 'rocket',
                        'mako', 'coolwarm', 'bwr', 'seismic', 'RdBu', 'Spectral', 'vlag']
        selected_palette = st.selectbox("Select a color palette", options=palette_list, key='pie_palette_select_reg')

        if st.button("Generate Pie Chart", key='pie_submit_reg'):
...
def countchart_plot(df):
    """
    Renders a count plot interface in Streamlit with conditional UI elements.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(cat_cols) > 1:
        selected = st.selectbox("Select a feature to count", cat_cols, key='count_x_reg')

        box = False
        huee, colorss = None, None

        if len(cat_cols) > 2:
            box = st.checkbox("Tick to activate hue", key='count_hue_check_reg')
            if box:
                palette_list = ['tab10', 'Set1', 'Set2', 'Set3', 'Paired', 'Pastel1', 'Dark2', 'colorblind', 'viridis',
                                'plasma', 'inferno', 'magma', 'cividis', 'Blues', 'Greens', 'Reds', 'YlOrRd', 'rocket', 'mako',
                                'coolwarm', 'bwr', 'seismic', 'RdBu', 'Spectral', 'vlag']
                ...
                with col1:
                    ...
                    huee = st.selectbox("Select Hue", options=hue_options, key='count_hue_select_reg')
                with col2:
                    st.write("❄️ Change the color theme.")
                    colorss = st.selectbox("Select Palette", options=palette_list, key='count_palette_select_reg')
        else:
            st.info("ℹ️ You need at least two categorical features to use the 'hue' option.")

        if st.button("Generate Count Plot", key='count_submit_reg'):
            if selected != "Choices":
                fig, ax = plt.subplots()
                if box and huee is not None and huee != "Choices":
...
def correlation_plot(df):
    """
    Renders a correlation heatmap interface in Streamlit.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(num_cols) >= 2:
        if st.button("Generate Correlation Heatmap", key='corr_submit_reg'):
...
def kde_plot(df):
    """
    Renders a KDE plot interface in Streamlit with conditional UI elements.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(num_cat) > 1:
        selected_x = st.selectbox("Select a feature", num_cat, key='kde_x_reg')

        box = False
        huee, colorss = None, None

        if len(alp_cat) > 1:
            box = st.checkbox("Tick to activate hue", key='kde_hue_check_reg')
            if box:
                palette_list = ['tab10', 'Set1', 'Set2', 'Set3', 'Paired', 'Pastel1', 'Dark2', 'colorblind', 'viridis',
                                'plasma', 'inferno', 'magma', 'cividis', 'Blues', 'Greens', 'Reds', 'YlOrRd', 'rocket',
                                'mako',
                ...
                with col1:
                    ...
                    huee = st.selectbox("Select Hue", options=alp_cat, key='kde_hue_select_reg')
                with col2:
                    st.write("❄️ Change the color theme of the chart.")
                    colorss = st.selectbox("Select Palette", options=palette_list, key='kde_palette_select_reg')
        else:
            st.info("ℹ️ Not enough categorical features available to use the 'hue' option.")

        if st.button("Generate KDE Plot", key='kde_submit_reg'):
...
def box_plot(df):
    """
    Renders a box plot interface with complex conditional UI.
    All widget keys are suffixed with _reg.
    """
    ...
    if len(num_cols) > 1:
        ...
        if len(cat_cols) > 1:
            with col1:
                selected_x = st.selectbox("Select X coordinate (Categorical)", cat_cols, key='box_x_reg')

        with col2:
            selected_y = st.selectbox("Select Y coordinate (Numerical)", num_cols, key='box_y_reg')

        box = False
        huee, colorss = None, None
        if len(cat_cols) > 2:
            box = st.checkbox("Tick to activate hue", key='box_hue_check_reg')
            if box:
                palette_list = ['tab10', 'Set1', 'Set2', 'Set3', 'Paired', 'Pastel1', 'Dark2', 'colorblind', 'viridis',
                                'plasma', 'inferno', 'magma', 'cividis', 'Blues', 'Greens', 'Reds', 'YlOrRd', 'rocket', 'mako',
                                'coolwarm', 'bwr', 'seismic', 'RdBu', 'Spectral', 'vlag']
                ...
                with c1:
                    ...
                    hue_options = [c for c in cat_cols if c != selected_x]
                    huee = st.selectbox("Select Hue", options=hue_options, key='box_hue_select_reg')
                with c2:
                    st.write("❄️ Change the color theme.")
                    colorss = st.selectbox("Select Palette", options=palette_list, key='box_palette_select_reg')
        else:
            st.info("ℹ️ You need at least two categorical features to use the 'hue' option.")

        if st.button("Generate Box Plot", key='box_submit_reg'):
            fig, ax = plt.subplots()
            if selected_x != "Choices" and selected_y != "Choices":
                if box and huee is not None and huee != "Choices":
...
def information(df):
    """
    Renders a comprehensive data summary and visualization tab interface.
    All widget keys are suffixed with _reg.
    """
    ...
    with st.expander("Perform Missing Value Analysis"):
        if(df.isna().sum().sum() > 0):
            ...
            selected_column = st.selectbox("Select a feature", options=new_columns_list, key='missing_value_select_reg')
