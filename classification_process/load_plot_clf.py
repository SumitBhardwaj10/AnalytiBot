...
def line_plot(df):
    ...
    - The main plot interface only appears if there are at least 2 numerical columns.
    - The 'hue' option only appears if there is at least 1 categorical column.
    """
    ...
    # --- Main Condition: Check for enough numerical features ---
    # The plotting interface will only be shown if there are 2 or more numerical columns.
    if len(num_cat) > 2:
        ...
        box = False # Default state for the checkbox

        # --- Hue Condition: Check for enough categorical features ---
        # The 'hue' option is only shown if there is at least one categorical column.
        if len(alp_cat) > 1:
        ...
        if st.button("Generate Line Plot", key='line_submit'):
            if selected_x != "Choices" and selected_y != "Choices":
                fig, ax = plt.subplots()
                # Check if hue should be applied
...
def histogram_plot(df):
    ...
    - The main plot interface only appears if there is at least 1 numerical column.
    - The 'hue' option only appears if there is at least 1 categorical column.
    """
...
def barchart_plot(df):
    ...
    - The main plot interface only appears if there is at least 1 numerical AND 1 categorical column.
    - The 'hue' option only appears if there are at least 2 categorical columns.
    """
    ...
    # --- Main Condition: Check for both numerical and categorical features ---
    if len(num_cols) > 1 and len(cat_cols) > 1:
        ...
        # --- Hue Condition: Check for at least two categorical features ---
        # One for the x-axis and at least one more for the hue
        if len(cat_cols) > 2:
            ...
            if box:
                ...
                with c1:
                    ...
                    # Ensure the selected X-axis category is not an option for hue
...
def piechart_plot(df):
    ...
    - The main plot interface only appears if there is at least 1 categorical column.
    """
    ...
    # --- Main Condition: Check for at least one categorical feature ---
    if len(cat_cols) > 1:
...
def correlation_plot(df):
    ...
    - The interface only appears if there are at least 2 numerical columns.
    """
    ...
    # --- Main Condition: Check for at least two numerical features ---
    if len(num_cols) >= 2:
...
def kde_plot(df):
    ...
    # --- Main Condition: Check for at least one numerical feature ---
    if len(num_cat) > 1:
        ...
        # --- Hue Condition: Check for at least one categorical feature ---
        if len(alp_cat) > 1:
...
def box_plot(df):
    ...
    - Requires at least 1 numerical column to show anything.
    - X-axis option appears if there's at least 1 categorical column.
    - Hue option appears if there are at least 2 categorical columns.
    """
    ...
    # --- Main Condition: Must have at least one numerical column ---
    if len(num_cols) > 1:
        selected_x = "Choices" # Default value if X-axis is not shown
        col1, col2 = st.columns(2)

        # X-axis option only appears if categorical columns are available
        if len(cat_cols) > 1:
            with col1:
        ...
        if st.button("Generate Box Plot", key='box_submit'):
            fig, ax = plt.subplots()
            # Case 1: Both X and Y are selected
            if selected_x != "Choices" and selected_y != "Choices":
                if box and huee is not None and huee != "Choices":
                ...
                plt.xticks(rotation=45)
                st.pyplot(fig)
            # Case 2: Only Y is selected
            elif selected_x == "Choices" and selected_y != "Choices":
                if box and huee is not None and huee != "Choices":
                    # Hue doesn't work well on a single-variable boxplot in seaborn, but we can plot by y
...
def information(df):
    ...
    with st.expander("Enter Visualization Mode"):
        ...
        with tab6:
            ...
            countchart_plot(df)  # Fixed this call
        with tab7:
